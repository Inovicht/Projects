...
def Import_Data(Test_Number,Excel_name,drive):
    ...
    path = "{}:\\DATA\\Test{}.txt".format(drive, Test_Number)
...
class Complementary_filter():
    ...
    def get_Fundamental_Data(self):
        self.Time = self.Data['Time'][self.Data.index[-1]]
        self.Samples = self.Data.index[-1]
        self.SamplesPerSecond = self.Samples/self.Time
        self.Frequency = 1/self.SamplesPerSecond
        print("Time: %s, Samples: %s, SamplesPerSecond: %s, Frequency: %s" % (self.Time, self.Samples, self.SamplesPerSecond, self.Frequency))
...
#rangedata = [5000,5600]  
rangedata = 0  
n_filter = 10 # the larger n is, the smoothlener curve will be (lfilter
...
if __name__ == "__main__":
    ...
    uf.get_Fundamental_Data()
    angle1 = uf.makeithappen(CF,"X1","Z1", "Y1",avg_n,n_filter)
    angle2 = uf.makeithappen(CF, "X2", "Z2", "Y2",avg_n, n_filter)
    angle3 = uf.makeithappen(CF, "X3", "Z3", "Y3",avg_n, n_filter,Inverse = 1)
    
    # Deleting unnecessary data from start-index
    start = 0
    end = uf.SamplesPerSecond*5
