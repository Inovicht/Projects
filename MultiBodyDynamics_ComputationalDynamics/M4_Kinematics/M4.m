...
n_rev = 3;              % How many revolutions
rev = (2*pi)/1.5;       % One Revolution
t = 0:0.01:rev*n_rev;   % Time-span
...
Acc_result = max(sqrt(abs(acc(1,:)).^2+abs(acc(2,:)).^2))
Acc_theta = max(16*abs(q_ddot(6,:)))
% Plotting Trajectory
figure(1)
plot(P1(1,:),P1(2,:),'r')
hold on
